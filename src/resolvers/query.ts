...
export const Query = {
    ...
    allStoreLinkNames: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return dispensaryModel.getAllStoreLinkNames(context)
        else  return throwUnauthorizedError()
    },
    ...
    allCustomersByDispensaryIdAndNameAndLicenseSearch: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return customerModel.getAllCustomersByDispensaryIdAndNameAndLicenseSearch(context, args.dispensaryId, args.searchQuery)
        else  return throwUnauthorizedError()
    },
    ...
    getMjProductRowsByNameSearch: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return productModel.getMjProductRowsByNameSearch(context, args)
        else  return throwUnauthorizedError()
    },
    getNonMjProductRowsByNameSearch: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return productModel.getNonMjProductRowsByNameSearch(context, args)
        else  return throwUnauthorizedError()
    },
    ...
    allPendingAdjustedPackagesByDispensaryId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return packageModel.getAllPendingAdjustedPackagesByDispensaryId(context, args.dispensaryId)
        else  return throwUnauthorizedError()
    },
    ...
    auditDiscrepancyPackages: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return packageModel.getAuditDiscrepancyPackages(context, args)
        else  return throwUnauthorizedError()
    },
    ...
    getNonMjPackagesByTransferId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return packageModel.getNonMjPackagesByTransferId(context, args)
        else  return throwUnauthorizedError()
    },
    getZeroMetrcQtyPackagesByDispensaryId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return packageModel.getZeroMetrcQtyPackagesByDispensaryId(context, args)
        else  return throwUnauthorizedError()
    },
    getTinyMetrcQtyPackagesByDispensaryId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return packageModel.getTinyMetrcQtyPackagesByDispensaryId(context, args)
        else  return throwUnauthorizedError()
    },
    getProductAndPackagesByNameSearch: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return packageModel.getProductAndPackagesByNameSearch(context, args)
        else  return throwUnauthorizedError()
    },
    ...
    getPurchaseLimitByDispensaryId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return purchaseLimit.getPurchaseLimitByDispensaryId(context, args.dispensaryId)
        else  return throwUnauthorizedError()  
    },
    getPurchaseLimitByPurchaseLimitType: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return purchaseLimit.getPurchaseLimitByPurchaseLimitType(context, args.dispensaryId)
        else  return throwUnauthorizedError()  
    },
    ...
    orderAmountInfo: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return orderModel.getOrderAmountInfo(context, args.id)
        else  return throwUnauthorizedError()  
    },
    getEditOrderByDrawerId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return orderModel.getEditOrderByDrawerId(context, args.drawerId)
        else  return throwUnauthorizedError()  
    },
    getEditOrderCountByDrawerId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return orderModel.getEditOrderCountByDrawerId(context, args.drawerId)
        else  return throwUnauthorizedError()  
    },
    ...
    allOrdersInfoIncludingAllTypesByDrawerId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return orderModel.getAllOrdersInfoIncludingAllTypesByDrawerId(context, args.drawerId)
        else  return throwUnauthorizedError()  
    },
    ...
    metrcAdjustmentReasonsByDispensaryId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return metrcItemCategoryModel.getMetrcAdjustmentReasonsByDispensaryId(context, args.dispensaryId)
        else  return throwUnauthorizedError()  
    },
    ...
    printSettingByDispensaryId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return reportModel.getPrintSettingByDispensaryId(context, args.dispensaryId)
        else  return throwUnauthorizedError()  
    },
    exitLabelByOrderId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return reportModel.getExitLabelByOrderId(context, args.orderId)
        else  return throwUnauthorizedError()  
    },
    exitLabelByPackageLabel: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return reportModel.getExitLabelByPackageLabel(context, args)
        else  return throwUnauthorizedError()  
    },
    receiptByOrderId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return reportModel.getReceiptByOrderId(context, args.orderId)
        else  return throwUnauthorizedError()  
    },
    actionHistory: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return reportModel.getActionHistory(context, args)
        else  return throwUnauthorizedError()  
    },
    totalTestResultByMetrcPackageId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return packageModel.getTotalTestResultByMetrcPackageId(context, args)
        else  return throwUnauthorizedError()  
    },
    totalTestResultByOrderId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return packageModel.getTotalTestResultByOrderId(context, args)
        else  return throwUnauthorizedError()  
    },
    getLastSyncHistoryByDispensaryId: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) return syncHistoryModel.getLastSyncHistoryByDispensaryId(context, args)
        else  return throwUnauthorizedError()  
    },
    ...
    insightSummaryReportByDayOfWeekAndHour: (_parent, args, context) => {
        if(context.role.includes(UserType.USER)) {
            return reportModel.getInsightSummaryReportByDayOfWeekAndHour(context, args)
        }
        else  return throwUnauthorizedError()  
    },
