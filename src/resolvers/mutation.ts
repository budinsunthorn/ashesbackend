...
export const Mutation = {
  createOrganization: async (_parent, _args, context) => {
    if (context.role.includes(UserType.SUPER_ADMIN_MANAGER_USER)) {
      try {
        ...
        creationData.orgLinkName = _args.input.name
          .replace(/\s+/g, "")
          .toLowerCase();
  ...
  createDispensary: async (_parent, _args, context) => {
    if (context.role.includes(UserType.ADMIN_MANAGER_USER)) {
      try {
        const creationData: any = _args.input;
        creationData.storeLinkName = _args.input.name
          .replace(/\s+/g, "")
          .toLowerCase();
    ...
    if (context.role.includes(UserType.MANAGER_USER)) {
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          const sendEmail = await userModel.sendEmailFromTeamForUserRegister(
            creation
          );
          return creation;
        });
        ...
        return context.prisma.$transaction(async (tx) => {
          const creation = await tx.customer.create({
            data: {
              ...
              birthday: new Date(_args.input.birthday).toLocaleDateString(
                "en-US"
              ),
              ...
              driverLicenseExpirationDate: new Date(
                _args.input.driverLicenseExpirationDate
              ).toLocaleDateString("en-US"),
              isMedical: _args.input.isMedical,
              medicalLicense: _args.input.medicalLicense.toUpperCase(),
              medicalLicenseExpirationDate: new Date(
                _args.input.medicalLicenseExpirationDate
              ).toLocaleDateString("en-US"),
  ...
  startDrawer: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      if (drawerCount > 0)
        return throwManualError(400, "Can not start duplicate register.");
    ...
    if (context.role.includes(UserType.MANAGER_USER)) {
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          const updateMjTaxApply = await taxSetting.updateTaxApply(
            context,
            _args.input.dispensaryId,
            TaxSettingApplyTo.MJ
          );
          const updateNmjTaxApply = await taxSetting.updateTaxApply(
            context,
            _args.input.dispensaryId,
            TaxSettingApplyTo.NMJ
          );
  ...
  fetchTestResultsByPackageId: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        let creation: any;
        const fetchTestResult = await metrcModel.fetchTestResultByPackageId(
          context,
          _args.input?.dispensaryId,
          _args.input?.packageId
        );
        ...
        console.log(
          "Test Result data imported for packageId : ",
          _args.input?.packageId,
          creation.count,
          "records"
        );
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          if (currentPackage.packageStatus != PackageStatus.FINISHED)
            return throwManualError(
              400,
              "Only Finished packages can be reactivated."
            );
          const paramData = [
            {
          ...
          const syncMetrc = await metrcModel.unFinishMetrcPackage(
            context,
            currentPackage.dispensaryId,
            paramData
          );
          if (syncMetrc != 200)
            return throwManualError(400, "Error syncing with Metrc.");
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          if (currentPackage.packageStatus != PackageStatus.ACTIVE)
            return throwManualError(
              400,
              "Only Active packages can be finished."
            );
          if (currentPackage.assignPackage.posQty > 0)
            return throwManualError(
              400,
              "To finish, package inventory must be zero."
            );
          ...
          if (currentPackage.packageId > 0) {
            const syncMetrc = await metrcModel.finishMetrcPackage(
              context,
              currentPackage.dispensaryId,
              paramData
            );
            if (syncMetrc != 200)
              return throwManualError(400, "Metrc Sync Failed.");
            const update = await tx.package.update({
              where: {
                id: _args.input.id,
              },
  ...
  createTransfer: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        inputData.ReceivedDateTime = format(
          new Date(),
        ...
        return context.prisma.$transaction(async (tx) => {
          const upsertSetting = await tx.printSetting.upsert({
            ...
            update: {
              ...
              fontFamily: _args.input.fontFamily,
            },
            create: {
              dispensaryId: _args.input.dispensaryId,
              isEnabled: _args.input.isEnabled,
  ...
  createOrderItem: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      currentTypeLimitValue.forEach((item) => {
        ...
        purchaseLimitAmountMap[item.purchaseLimitType] =
          item.purchaseLimitAmount;
      });
      ...
      if (product.itemCategory.containMj) {
        if (!product.itemCategory.purchaseLimitType)
          return throwManualError(
            400,
            "Please set Limit Type for " + product.itemCategory.name
          );
        if (currentTypeLimitValue.length > 0) {
          const purchaseLimit = order.OrderItem.filter(
            (item) => item.mjType === OrderMjType.MJ
          ).reduce((acc, item) => {
            const purchaseLimitType =
              item.product.itemCategory.purchaseLimitType;
          ...
          const standardLimitAmount =
            purchaseLimitAmountMap[product.itemCategory.purchaseLimitType];
          const standardLimitUnit =
            limitUnitMap[product.itemCategory.purchaseLimitType];
          const limitWeightType =
            limitWeightMap[product.itemCategory.purchaseLimitType];
          ...
          if (product.productUnitOfMeasure == ProductUnitOfMeasure.ea) {
            ...
            const itemWeightUnit =
              limitWeightType == LimitWeight.UnitWeight
                ? product.unitOfUnitWeight
                : product.unitOfNetWeight;
          ...
          if (limitAmountAfterAdded > standardLimitAmount)
            return throwManualError(
              400,
              "Exceeded " + product.itemCategory.purchaseLimitType + " limit"
            );
        }
      }

      const applyTo = product.itemCategory.containMj
        ? TaxSettingApplyTo.MJ
        : TaxSettingApplyTo.NMJ;
      ...
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          let baseAmount =
            ...
            setFourDecimals(_args.input.price);
          let costAmount =
            setFourDecimals(_args.input.quantity) *
            setFourDecimals(_args.input.cost);
          // Get Discount Info
          ...
          if (discountHistory.length > 0) {
            discountedAmount = await discountModel.setDiscountForOrderItems(
              tx,
              orderId,
              discountHistory[0].discountMethod,
              discountHistory[0].value,
              setFourDecimals(baseAmount)
            );
          }
          ...
          if (loyaltyHistory.length > 0) {
            loyaltyAmount = await loyaltyModel.setLoyaltyForOrderItems(
              tx,
              orderId,
              loyaltyHistory[0].loyaltyType,
              loyaltyHistory[0].loyaltyWorth,
              loyaltyHistory[0].value,
              setFourDecimals(baseAmount)
            );
          }
  ...
  createOrderItemForReturn: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      if (order.orderType === OrderType.SALE)
        return throwManualError(
          400,
          "The Order #" + _args.input.orderId + " is SALE type."
        );
  ...
  completeOrder: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        await Promise.all(
          items.map((item: any) =>
            updateStock(item.packageLabel, item.quantity)
          )
        );
        ...
        const deletionCustomerInQueue =
          await context.prisma.customerQueue.deleteMany({
            where: {
              customerId: _args.input.customerId,
            },
          });
        ...
        if (currentOrder.mjType === OrderMjType.MJ) {
          const metrcReportResult = await metrcModel.postMetrcReceiptByOrderId(
            context,
            _args.input.dispensaryId,
            _args.input.orderId
          );
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        for (let i = 0; i < orders.length; i++) {
          const updateOrder = await metrcModel.unFinalizeOrderByMetrcId(
            context,
            _args.input.dispensaryId,
            orders[i].metrcId
          );
  ...
  finalizeMetrcOrderDataByOrderDate: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        for (let i = 0; i < orders.length; i++) {
          const updateOrder = await metrcModel.finalizeOrderByMetrcId(
            context,
            _args.input.dispensaryId,
            orders[i].metrcId
          );
  ...
  updateMetrcOrderDataByOrderDate: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        for (let i = 0; i < orders.length; i++) {
          const updateTax = await orderitemModel.updateTaxHistoryForOrder(
            context,
            orders[i].id
          );
          const updateOrder = await metrcModel.putMetrcReceiptByOrderId(
            context,
            _args.input.dispensaryId,
            orders[i].id
          );
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          const unSyncResult = await metrcModel.delMetrcReceipt(
            context,
            order.dispensaryId,
            metrcId,
            _args.input.id
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          await loyaltyModel.setLoyaltyForOrderItems(
            tx,
            _args.input.orderId,
            _args.input.loyaltyType,
            _args.input.loyaltyWorth,
            _args.input.value,
            0
          );
  ...
  changePassword: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        const isOldpasswordTrue = await userModel.checkOldpassword(
          context,
          _args.userId,
          _args.oldPassword
        );
  ...
  cancelOrder: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      if (order.status !== OrderStatus.EDIT)
        return throwManualError(
          400,
          "Only EDIT status orders can be cancelled."
        );
      try {
        return context.prisma.$transaction(async (tx) => {
  ...
  voidOrder: async (_parent, _args, context) => {
    if (context.role.includes(UserType.MANAGER_USER)) {
      ...
      if (order.status !== OrderStatus.PAID)
        return throwManualError(
          400,
          "The order #" + _args.input.id + " can not be voided."
        );
      ...
      try {
        ...
        const metrcReportResult = await metrcModel.delMetrcReceipt(
          context,
          order.dispensaryId,
          order.metrcId,
          _args.input.id
  ...
  endDrawer: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      if (editCount > 0)
        return throwManualError(
          400,
          "There are EDIT orders in the current Register"
        );
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          const updating = await tx.discount.update({
            ...
            data: {
              ...
              manualDiscountLimitPercent:
                _args.input.manualDiscountLimitPercent,
              color: _args.input.color,
            },
          });
          const actionHistory = await tx.actionHistory.create({
  ...
  updateCustomerByOrderId: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      if (order.status !== OrderStatus.EDIT)
        return throwManualError(
          400,
          "Order #" + _args.input.orderId + " is on " + order.status + " status"
        );
    ...
    if (context.role.includes(UserType.MANAGER_USER)) {
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          if (transfer.isMJ == true || transfer.assignedPackageCount > 0)
            return null;
  ...
  syncMetrcItemCategory: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        const metrcDataInput = await metrcModel.getMetrcItemCategoryData(
          _args,
          context
        );
        ...
        console.log(
          "Metrc Item Category data imported:",
          creation.count,
          "records"
        );
        return creation;
  ...
  syncAdjustmentReasons: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        const metrcDataInput = await metrcModel.getMetrcAdjustmentReasonsData(
          _args,
          context
        );
        ...
        console.log(
          "Metrc Adjustment Reasons data imported:",
          creation.count,
          "records"
        );
        return creation;
  ...
  reconcilePackageWithMetrcByAdjustId: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        if (pendingAdjust.syncMetrc === true)
          return throwManualError(
            400,
            "Current record was synced in the past."
          );
        if (pendingAdjust.needMetrcSync === false)
          return throwManualError(400, "This record can not be synced.");
        const syncMetrc = await metrcModel.postAdjustOnePackage(
          context,
          _args.input.dispensaryId,
          pendingAdjust
        );
  ...
  importSuppliers: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        let creation: any;
        const supplierDataInput =
          await supplierModel.getSupplierDataFromPackages(_args, context);
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        for (let i = 0; i < deliveryPackages.length; i++) {
          console.log(
            i,
            deliveryPackages[i].packageLabel,
            deliveryPackages[i].transfer.id
          );
          const existingAssignPackage =
            await context.prisma.assignPackage.findUnique({
          ...
          if (existingAssignPackage) {
            ...
            console.warn(
              `AssignPackage with packageLabel ${deliveryPackages[i].packageLabel} not found.`
            );
      ...
      try {
        return context.prisma.$transaction(
          async (tx) => {
            ...
            const lastModifiedStart = syncHistory
              ? syncHistory.createdAt
              : "defaultStartDate";

            const activePackageData =
              await metrcModel.getMetrcActivePackageData(
                _args,
                context,
                lastModifiedStart
              );
            const inActivePackageData =
              await metrcModel.getMetrcInactivePackageData(
                _args,
                context,
                lastModifiedStart
              );
            const allNewUpdatedPackageData =
              activePackageData.concat(inActivePackageData);
            ...
            for (let i = 0; i < allNewUpdatedPackageData.length; i++) {
              ...
              const packageUpdate = await tx.package.upsert({
                where: {
                  packageLabel: allNewUpdatedPackageData[i].packageLabel,
                },
            ...
            const isSuccess =
              allNewUpdatedPackageData.length > 0 ? true : false;

            console.log(
              "Metrc package data imported:",
              allNewUpdatedPackageData.length,
              "records"
            );
            return returnCount;
          },
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        for (const item of packagesList) {
          const metrcPackage = await metrcModel.getMetrcPackageInfoByPackageId(
            _args,
            context,
            item.packageId
          );
          if (metrcPackage.OriginalPackageQuantity > 0) {
            await context.prisma.package.update({
              ...
              data: {
                originalQty: truncateToTwoDecimals(
                  metrcPackage.OriginalPackageQuantity
                ),
              },
            });
          }
        ...
        console.log(
          "Metrc package original quantity updated:",
          returnCount.count,
          "records"
        );
        return returnCount;
        ...
        return context.prisma.$transaction(
          async (tx) => {
            ...
            const lastModifiedStart = syncHistory
              ? syncHistory.createdAt
              : "defaultStartDate";
            const transferDataInput = await metrcModel.getMetrcIncomingTransfer(
              _args,
              context,
              lastModifiedStart
            );
            ...
            console.log(
              "Metrc incoming transfer data synced:",
              transferDataInput.length,
              "records from ",
              lastModifiedStart
            );
            ...
            for (let i = 0; i < transferDataInput.length; i++) {
              onePageData =
                await metrcModel.getMetrcDeliveryPackagesByDeliveryId(
                  _args,
                  context,
                  transferDataInput[i].deliveryId
                );
              deliveryPackageData.push(onePageData);
            }

            deliveryPackageData = await deliveryPackageData.flat();
            const deliveryPackageFilterArray = await deliveryPackageData.map(
              (b) => b.packageId
            );
            const deliveryPackageDataResult = await deliveryPackageData.filter(
              (a) => deliveryPackageFilterArray.includes(a.packageId)
            );
            // console.log(deliveryPackageDataResult)

            for (let i = 0; i < deliveryPackageDataResult.length; i++) {
              const packageId = parseInt(
                deliveryPackageDataResult[i].packageId
              );
              try {
                // console.log("packageId>>>>>> ", packageId)
                // Check if the related package exists
                if (packageListArray.includes(packageId)) {
                  const deliveryPackageCreate =
                    await tx.deliveryPackages.upsert({
                      where: {
                        packageId: packageId,
                      },
            ...
            console.log(
              "Metrc deliver packages data synced:",
              deliveryPackageDataResult.length,
              "records from ",
              lastModifiedStart
            );
            let returnCount: any = {
  ...
  syncDeliveryPackages: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        let creation: any;
        const syncedCount = await metrcModel.syncMetrcDeliveryPackages(
          _args,
          context
        );
        console.log(
          "Metrc delivery package data imported:",
          syncedCount,
          "records"
        );
        let returnCount: any = {
  ...
  syncMetrcReceipts: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        const lastModifiedStart =
          syncHistory.length > 0
            ? syncHistory[0].createdAt
            : "defaultStartDate";
        let orderIdList: any = [];
        const orderDataInput = await metrcModel.getMetrcAllReceiptData(
          _args,
          context,
          lastModifiedStart
        );
        ...
        const metrcInfo = await metrcModel.getMetrcInfoByDispensaryId(
          context,
          _args.input.dispensaryId
        );
        ...
        if (lastModifiedStart !== "defaultStartDate")
          where.id = {
            in: orderIdList,
          };
        ...
        for (let i = 0; i < orderList.length; i++) {
          const detailData = await metrcModel.getMetrcReceiptByIdDataByParams(
            metrcApiEndpoint,
            metrcApiKey,
            cannabisLicense,
            orderList[i].metrcId
  ...
  syncMetrcReceiptsDetail: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        const syncedCount = await metrcModel.getMetrcReceiptDetailData(
          _args,
          context
        );
  ...
  importGrowflowCustomer: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      try {
        await fs
          .createReadStream("./src/migration/growflow/customers.csv")
          .pipe(csv())
            ...
            customers = results.map((customerRecord) => {
              return {
                ...
                driverLicenseExpirationDate:
                  customerRecord.CustomerStateLicenseExpiration,
                isMedical: true,
                medicalLicense: customerRecord.MedicalLicenseNumber,
                medicalLicenseExpirationDate:
                  customerRecord.LicenseEffectiveEndDate,
    ...
    if (context.role.includes(UserType.USER)) {
      ...
      try {
        ...
        for (let i = 0; i < itemCategory.length; i++) {
          itemCategoryList[
            itemCategory[i].name.replace(/\s/g, "").toLowerCase()
          ] = itemCategory[i].id;
        }
        ...
        await fs
          .createReadStream("./src/migration/growflow/products.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
  ...
  importDutchieProduct: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      try {
        ...
        await fs
          .createReadStream("./src/migration/dutchie/catalog.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
            for (let i = 0; i < results.length; i++) {
              try {
                ...
                const product = {
                  ...
                  productUnitOfMeasure:
                    unitTypeTransfer[results[i].Type] || "ea",
                  unitWeight: 0,
                  netWeight: parseFloat(results[i].Netweight) || 0,
                  isConnectedWithPackage: false,
                  otherPosUniqueId: results[i].SKU.replace(
                    /\s/g,
                    ""
                  ).toLowerCase(),
                };
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        for (let i = 0; i < products.length; i++) {
          productIdByName[products[i].name.replace(/\s/g, "").toLowerCase()] =
            products[i].id;
        }
        ...
        for (let i = 0; i < deliveryPackages.length; i++) {
          deliveryIdByPackageLabel[deliveryPackages[i].packageLabel] =
            deliveryPackages[i].deliveryId;
        }
        for (let i = 0; i < packages.length; i++) {
          metrcQtyByPackageLabel[packages[i].packageLabel] =
            packages[i].Quantity;
        }
        // console.log("deliveryIdByPackageLabel>>>>>>>>>>> ", deliveryIdByPackageLabel)
        await fs
          .createReadStream("./src/migration/growflow/packages.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
            for (let i = 0; i < results.length; i++) {
              ...
              if (!results[i].MetrcTag) {
                ...
                const updatePackageLabelForNonMjPackage =
                  await context.prisma.package.update({
                    where: {
                      id: nonMjPackageId,
                    },
              ...
              const pLabel = results[i].MetrcTag
                ? results[i].MetrcTag
                : nonMjPackageId;
              const posQty = results[i].MetrcTag
                ? parseFloat(metrcQtyByPackageLabel[pLabel] || 0)
                : parseFloat(results[i].CurrentQty || 0);
  ...
  importDutchieAssignPackage: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        // console.log("deliveryIdByPackageLabel>>>>>>>>>>> ", deliveryIdByPackageLabel)
        await fs
          .createReadStream("./src/migration/dutchie/inventory.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
            for (let i = 0; i < results.length; i++) {
              ...
              const posQty =
                results[i].Iscannabis == "Yes"
                  ? parseFloat(metrcQtyByPackageLabel[pLabel] || 0)
                  : parseFloat(results[i].Quantity || 0);
              if (packageLabels.includes(pLabel)) continue;
              const assignOne = {
                dispensaryId: _args.input.dispensaryId,
                userId: _args.input.userId,
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        return context.prisma.$transaction(async (tx) => {
          ...
          const deltaQty = setFourDecimals(
            setFourDecimals(_args.input.newQty) - metrcQty
          );
          const dd = {
            dispensaryId: _args.input.dispensaryId,
            packageLabel: _args.input.packageLabel,
    ...
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        for (let i = 0; i < orders.length; i++) {
          ...
          const customerId = customerIdListByLicense[
          ...
          netAmount[orders[i].metrcId] = truncateToTwoDecimals(
            orders[i].cashAmount
          );
        }
        ...
        let results: any = [];
        await fs
          .createReadStream("./src/migration/growflow/orders.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
            for (let i = 0; i < results.length; i++) {
              ...
              console.log(
                "ExternalId AMount>>> ",
                netAmount[parseInt(results[i].ExternalId)]
              );
              ...
              const netTotal = results[i].ExternalId
                ? netAmount[parseFloat(results[i].ExternalId)]
                : 0;
  ...
  adjustDutchieOrder: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        let results: any = [];
        await fs
          .createReadStream("./src/migration/dutchie/transactions.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
            for (let i = 0; i < results.length; i++) {
              // console.log("number: ", i, "  >>  ", results.length)
              const timeIndex = new Date(results[i].Date)
                .toISOString()
                .slice(0, 19);
          ...
          .on("end", async () => {
            for (let i = 0; i < results.length; i++) {
              productNameByMetrcName[
                results[i].MetrcName.replace(/\s/g, "").toLowerCase()
              ] = results[i].Product.replace(/\s/g, "").toLowerCase();
              costByPackageLabel[results[i].MetrcTag] = parseFloat(
                results[i].CostPerItem.replace(/[$,]/g, "")
              );
            }
            const orderItems = await context.prisma.orderItem.findMany({
  ...
  adjustDutchieOrderItem: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      try {
        ...
        for (let i = 0; i < products.length; i++) {
          productIdBySku[
            products[i].otherPosUniqueId.replace(/\s/g, "").toLowerCase()
          ] = products[i].id;
        }
        ...
        await fs
          ...
          .on("end", async () => {
            for (let i = 0; i < results.length; i++) {
              skuByPackageLabel[results[i].PackageID] = results[i].SKU.replace(
                /\s/g,
                ""
              ).toLowerCase();
              costByPackageLabel[results[i].PackageID] = parseFloat(
                results[i].Cost.replace(/[$,]/g, "")
              );
            }
  ...
  importDutchieCustomer: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      try {
        await fs
          .createReadStream("./src/migration/dutchie/customers.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
            // console.log(results);
            customers = results.map((customerRecord) => {
              return {
                ...
                driverLicenseExpirationDate:
                  customerRecord.DriversLicenseExpiration,
                isMedical: true,
            ...
            for (let i = 0; i < customers.length; i++) {
              if (
                customers[i].name == "" ||
                customers[i].medicalLicense == null
              )
                continue;
              const customerUpsert = await context.prisma.customer.upsert({
                where: {
  ...
  importDutchieCustomerLoyalty: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      try {
        await fs
          .createReadStream("./src/migration/dutchie/customerLoyalty.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
            // console.log(results);
            ...
            for (let i = 0; i < customers.length; i++) {
              ...
              console.log(
                customers[i].customerName,
                parseFloat(customers[i].loyaltyPoint)
              );
            }
  ...
  importDutchieVendor: async (_parent, _args, context) => {
    if (context.role.includes(UserType.USER)) {
      ...
      try {
        await fs
          .createReadStream("./src/migration/dutchie/vendors.csv")
          .pipe(csv())
          .on("data", (data) => results.push(data))
          .on("end", async () => {
            // console.log(results);
            ...
            for (let i = 0; i < vendors.length; i++) {
              ...
              console.log(
                ...
                parseFloat(vendors[i].Vendorcode)
              );
            }
